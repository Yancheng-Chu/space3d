...
class Box {
 ...
 //     assert(min < max);
	...
	// to calculate each axis between two boxes
	//
	 bool overlap(const Box &box) {
		 Vector3 p = box.parameters[0];
		 Vector3 p2 = box.parameters[1];
		 return (parameters[0].x() <= p2.x() && parameters[1].x() >= p.x()) &&
			 (parameters[0].y() <= p2.y() && parameters[1].y() >= p.y()) &&
			 (parameters[0].z() <= p2.z() && parameters[1].z() >= p.z());
	}
